...
/**
 * The orientation provider that delivers the relative orientation from the
...
public class CalibratedGyroscopeProvider extends OrientationProvider {
     ...
     * @param glassSensor
     */
    public CalibratedGyroscopeProvider(Context context, USBSensorCallback glassSensor) {
    ...
    public void onSensorChanged(float[] accSensorData, float[] gyroSensorData, float[] magSensorData) {
        ...
        if (timestamp != 0) {
            final float dT = (getTimeNanos() - timestamp) * NS2S;
            // Axis of the rotation sample, not normalized yet.
            float axisX = gyroSensorData[0];
            float axisY = gyroSensorData[1];
            float axisZ = gyroSensorData[2];
            ...
            synchronized (synchronizationToken) {
                ...
                getRotationMatrixFromVector(currentOrientationRotationMatrix.matrix,
